<div x-data="{mostrarCarrinho: @entangle('mostrarCarrinho')}">
        ...
        <div class="flex w-full mt-5 flex-column">
            @if($carrinhos->count() > 0)
                @foreach($carrinhos as $carrinho)
                    <div class="w-full">
                        <h4>Reserva: {{ $carrinho->reserva->fazenda->nome_fazenda }}</h4>
                    </div>
                    @foreach($carrinho->produtos as $produto)
                        <div class="flex py-3 border-b border-solid flex-collumn border-slate-400">
                            <div class="w-1/3">
                                <img class="w-full rounded-md shadow-md" src="{{ asset($produto->produtable->preview) }}" alt="">
                            </div>
                            <div class="relative w-2/3 pl-2">
                                <div>
                                    <span class="text-[15px] text-[#626262] font-semibold">{{ $produto->produtable->nome }}</span>
                                </div>
                                <div class="mt-[-5px]">
                                    <span class="text-[12px] text-[#626262] font-medium">RGD: {{ $produto->produtable->registro }}</span>
                                </div>
                                <div class="mt-[-5px]">
                                    <span class="text-[14px] text-[#626262] font-semibold">R${{ number_format($produto->produtable->preco, 2, ",", ".") }}</span>
                                </div>
                                
                                <i class="absolute right-0 duration-300 text-danger bottom-2 fas fa-trash hover:scale-110 cpointer" wire:click="removerProduto({{ $carrinho->id }}, {{ $produto->id }})"></i>
                            </div>
                        </div>
                    @endforeach
                @endforeach
