<div x-data="{ count: 0 }" x-ref="iconeCarrinho" class="w-full relative" @click="openSideCart = !openSideCart"
    x-init="document.addEventListener('atualizaContagemLotes', (event) => {
        count = event.detail.cont
    })">

    <svg class="mx-auto fill-[#80828b] hover:fill-[#5b5d63] transition duration-150" id="Grupo_3723" data-name="Grupo 3723"
        xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="36.992" height="37"
        ...
        <g id="Grupo_3682" data-name="Grupo 3682" clip-path="url(#clip-path)">
            ...
            <ellipse id="Elipse_305" data-name="Elipse 305" cx="3.179" cy="3.179" rx="3.179" ry="3.179"
                transform="translate(26.182 28.156)" />
            <ellipse id="Elipse_306" data-name="Elipse 306" cx="3.179" cy="3.179" rx="3.179" ry="3.179"
                transform="translate(6.185 28.156)" />
...
@push('scripts')
    <script>
        // Acesse o elemento DOM do componente Alpine usando x-ref
        const myComponentElement = document.querySelector('[x-ref="iconeCarrinho"]');

        // Adicione um event listener ao elemento DOM
        document.addEventListener('atualizaContagemLotes', (event) => {
            Alpine.executeMethod(myComponentElement, 'setCount', event.detail.cont);
        });
    </script>
@endpush
