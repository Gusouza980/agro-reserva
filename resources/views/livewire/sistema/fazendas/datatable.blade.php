<div class="w-full">
    <div class="w-full">
        ...
        <div class="w-full flex mb-4 card p-4">
            <div class="w-full md:w-1/5">
                <label class="block">
                    <span>Pesquisar por nome da fazenda</span>
                    <span class="relative mt-1.5 flex">
                        <input wire:model.debounce.1000ms="filtro.texto" class="input-base"
                            placeholder="Digite sua busca" type="text">
                    </span>
                </label>
            </div>
        </div>
        <div class="min-w-full overflow-x-auto rounded-none card is-scrollbar-hidden" x-data="pages.tables.initExample1">
            <table class="w-full text-left is-hoverable" style="vertical-align: middle;">
                <thead>
                    <tr>
                        <th
                            class="px-4 py-3 font-semibold uppercase whitespace-nowrap bg-slate-200 text-slate-800 lg:px-5">
                            #</th>
                        <th
                            class="px-4 py-3 font-semibold uppercase whitespace-nowrap bg-slate-200 text-slate-800 lg:px-5 w-[200px]">
                        ...
                        <tr class="border-transparent border-y border-b-slate-200 dark:border-b-navy-500">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap sm:px-5 w-[100px]">
                                <label for="input_logo_{{ $fazenda->id }}" class="cursor-pointer">
                                    <img src="{{ \Util::getImagemOuPadrao($fazenda->logo) }}" loading="lazy"
                                        class="w-[100px] rounded-md" alt="" width="100">
                                </label>
                                <input id="input_logo_{{ $fazenda->id }}" style="display: none;" type="file"
                                    wire:model="logos.{{ $fazenda->id }}" accept="image/*">
                            </td>
