            ...
            <tbody>
                @foreach ($usuarios as $usuario)
                    <tr class="border-transparent border-y border-b-slate-200 dark:border-b-navy-500">
                        <td class="px-4 py-3 whitespace-nowrap sm:px-5">#{{ $usuario->id }}</td>
                        <td class="px-4 py-3 whitespace-nowrap sm:px-5">
                            <label for="input_preview_{{ $usuario->id }}" class="cursor-pointer">
                                <img src="{{ ($usuario->foto) ? asset($usuario->foto) : asset("imagens/sem-foto.jpg") }}" alt="" width="80">
                            </label>
                            <input id="input_preview_{{ $usuario->id }}" style="display: none;" type="file" wire:model="arquivos.{{ $usuario->id }}" accept="image/*">
                        </td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap sm:px-5">
                            <button class="px-3 py-2 text-white bg-gray-600 rounded-lg">{{ \Acessos::$niveis[$usuario->acesso] }}</button>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap sm:px-5">
                                    ...
                                    <div
                                        ...
                                        <ul>
                                            <li @click="isShowPopper = false">
                                                <a onclick="Livewire.emit('carregaModalEdicaoUsuario', {{ $usuario->id }})"
                                        ...
                                        <ul>
                                            <li @click="isShowPopper = false">
                                                <a onclick="Livewire.emit('carregaModalExcluirUsuario', {{ $usuario->id }})"
