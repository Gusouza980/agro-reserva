                ...
                <tbody>
                    @foreach($reservas as $reserva)
                        <tr class="border-transparent border-y border-b-slate-200 dark:border-b-navy-500">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap sm:px-5">
                                <select class="w-full px-3 py-2 bg-transparent border rounded-lg form-input peer border-slate-300 pl-9 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" onchange="Livewire.emit('atualizaValor', {{ $reserva->id }}, 'compra_disponivel', this.value)">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap sm:px-5">
                                <select class="w-full px-3 py-2 bg-transparent border rounded-lg form-input peer border-slate-300 pl-9 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" onchange="Livewire.emit('atualizaValor', {{ $reserva->id }}, 'encerrada', this.value)">
                                    <option value="1" @if($reserva->encerrada) selected @endif>Sim</option>
                                    <option value="0" @if(!$reserva->encerrada) selected @endif>Não</option>
                                </select>
                            </td>
                                    ...
                                    <div x-ref="popperRoot" class="popper-root" :class="isShowPopper && 'show'" wire:ignore.self>
                                        <div
                                            ...
                                            <ul>
                                                <li @click="isShowPopper = false">
                                                    <a onclick="Livewire.emit('carregaModalEdicaoUsuario', {{ $reserva->id }})"
                                                        class="flex items-center h-8 px-3 pr-12 font-medium tracking-wide transition-all outline-none cursor-pointer hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100">Embriões</a>
                                                </li>
                                            </ul>
                                            <ul>
                                                <li @click="isShowPopper = false">
                                            ...
                                            <ul>
                                                <li @click="isShowPopper = false">
                                                    <a onclick="Livewire.emit('carregaModalEdicaoUsuario', {{ $reserva->id }})"
                                                        class="flex items-center h-8 px-3 pr-12 font-medium tracking-wide transition-all outline-none cursor-pointer hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100">Mapa de Compra</a>
                                                </li>
                                            </ul>
                                            <div class="h-px my-1 bg-slate-150 dark:bg-navy-500"></div>
                                            <ul>
                                                <li @click="isShowPopper = false">
                                                    <a onclick="Livewire.emit('carregaModalExcluirUsuario', {{ $reserva->id }})"
