<div class="mx-auto vitrine-animais">
    @if($configuracao->mostrar_lotes_destaque)
        ...
        @if(!$reserva_selecionada)
            @foreach(\App\Models\Reserva::where([["ativo", true], ["aberto", true], ["encerrada", false]])->get() as $reserva_aberta)
                ...
                @if($lotes_destaque->count() > 0)
                    ...
                    <div class="mt-4 row">
                        <div class="col-12">
                            <div class="slick-lotes" id="slick{{ $reserva_aberta->id }}">
        ...
        @else
            ...
            @endphp
            @if($lotes_destaque->count() > 0)
                <div class="mt-5 text-center col-12 text-header-index d-lg-block">
                    <h5> {{ mb_strtoupper(__('messages.home.animais_em_destaque')) }} - {{ mb_strtoupper($reserva_selecionada->fazenda->nome_fazenda) }} </h5>
                </div>
                
                <div class="row">
                    <div class="col-12">
                        <div class="slick-lotes" id="slick{{ $reserva_selecionada->id }}">
                            @foreach ($lotes_destaque as $lote)
                                <div class="px-0 py-2 mt-4 caixa-lote-home cpointer" onclick="window.location.href = '{{route('fazenda.lote', ['fazenda' => $lote->reserva->fazenda->slug, 'lote' => $lote, 'reserva' => $lote->reserva])}}'">
                                    {{-- <div data-aos-duration="500" class="px-0 py-2 mx-0 mt-4 lazy mt-lg-0 mx-lg-2"> --}}
                                    <div class="caixa-lote-home-imagem"
        ...
        <script>
            $(document).ready(function(){
                $("#slick{!! $reserva_selecionada->id !!}").slick({
                    ...
                    // normal options...
                    slidesToShow: @if($reserva_selecionada->lotes->where("ativo", true)->where("reservado", false)->count() < 4) $reserva_selecionada->lotes->where("ativo", true)->where("reservado", false)->count() @else 4 @endif,
                    infinite: true,
                    // dots: true,
                    adaptiveHeight: true,
                    arrows: true,
