<div class="w-full px-0 py-5" style="position: relative; min-height: 600px;">
    <div class="relative mx-auto w1200">
                    ...
                    <div class="inline-block mx-[6px] slide-item"
                        ...
                        @if ($reserva->catalogo)
                            <a href="{{ asset($reserva->catalogo) }}" target="_blank"
                                title="Catálogo - {{ $reserva->fazenda->nome_fazenda }}"
                                class="w-10 h-10 rounded-full flex items-center justify-center bg-orange-600 text-white absolute top-0 right-0 transition duration-200 hover:scale-105"><i
                                    class="fas fa-file fa-lg"></i></a>
                        @endif
                        <img src="{{ asset($reserva->imagem_card) }}"
                            class="w-100 @if ($reserva->encerrada) brightness-[0.30]" @endif alt="">
                            ...
                            <div class="text-center">
                                @if (!$reserva->encerrada)
                                    @if ($reserva->mostrar_datas)
                                        <div>
                                            <b class="font-montserrat text-[16px] text-white">
                                        ...
                                        <div class="mt-2">
                                            <h3 class="font-montserrat text-white text-[26px] font-bold">
                                                @if (!$reserva->compra_disponivel)
                                                    {{ date('d/m/Y', strtotime($reserva->inicio)) }}
                                                @else
                                                    {{ date('d/m/Y', strtotime($reserva->fim)) }}
                                                @endif
                                            </h3>
                                        </div>
                                    @else
                                ...
                                @if ($reserva->aberto)
                                    <div class="mt-3">
                                        <a href="{{ route('fazenda.lotes', ['fazenda' => $reserva->fazenda->slug, 'reserva' => $reserva->id]) }}"
                                            name="" id=""
                                            class="px-[30px] py-[10px] @if ($reserva->encerrada) border-2 border-[#E8521B] @else bg-[#E8521B] @endif text-[#FFFFFF] rounded-[6px] transition duration-300 font-montserrat text-[17px] font-bold hover:text-white hover:bg-[#b83f13]"
                                            href="#" role="button">Ver Lotes</a>
                                    </div>
                                @endif
                            </div>
                        </div>
        ...
        <img src="{{ asset('imagens/slide-lotes-arrow-left.png') }}" id="slide-reservas-ativas-left"
            class="absolute cpointer hidden md:block" height="25" style="top: calc(50% - 25px); left: 0px;"
            alt="">
        <img src="{{ asset('imagens/slide-lotes-arrow-right.png') }}" id="slide-reservas-ativas-right"
            class="absolute cpointer hidden md:block" height="25" style="top: calc(50% - 25px); right: 0px;"
            alt="">
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#slide-reservas-ativas-left").click(function() {
                $("#slide-reservas-ativas").animate({
                    scrollLeft: $("#slide-reservas-ativas").scrollLeft() - item_width
                }, function() {
                    updateButtons();
                });
            });
