<div class="w-full pl-1 md:pl-0 py-2 pb-5" style="position: relative;">
    <div class="relative mx-auto w1200">
                ...
                @foreach ($lotes->where('ativo', true) as $lote)
                    <div class="inline-block py-2 slide-item cpointer">
                        <div class="caixa-lote-home-imagem"
                            onclick="window.location.href = '{{ route('fazenda.lote', ['fazenda' => $lote->fazenda->slug, 'lote' => $lote, 'reserva' => $lote->reserva]) }}'"
                        ...
                        <div class="px-4 mt-3 flex items-center justify-start">
                            ...
                            <div>
                                <div class="flex justify-start items-center">
                                    <div>
                                        <button class="px-2 badge-lote-home">LOTE
                                            {{ str_pad($lote->numero, 2, '0', STR_PAD_LEFT) }}@if ($lote->letra)
                                                {{ $lote->letra }}
                                            @endif
                                        </button>
                                    </div>
                                ...
                                <div class="mt-2 text-left caixa-lote-home-text relative z-10">
                                    @if (!$reserva || $reserva->modalidade == 0)
                                        <span>
                                            @if ($lote->preco > 0)
                                                {{ $lote->reserva->max_parcelas . "x de R$" . number_format($lote->preco / $lote->reserva->max_parcelas, 2, ',', '.') }}
                                            @else
                                                {{ $lote->reserva->desconto }}% de desconto no
                                                <br>pagamento à vista
                                            @endif
                                        </span>
                                    @else
        ...
        <img src="{{ asset('imagens/slide-lotes-arrow-left.png') }}"
            id="slide-lotes-destaque-{!! $identificador !!}-left" class="absolute cpointer hidden md:block"
            height="25" style="top: calc(50% - 25px); left: 0px;" alt="">
        <img src="{{ asset('imagens/slide-lotes-arrow-right.png') }}"
            id="slide-lotes-destaque-{!! $identificador !!}-right" class="absolute cpointer hidden md:block"
            height="25" style="top: calc(50% - 25px); right: 0px;" alt="">
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateButtons() {
                var min = 0;
                var max = $("#slide-lotes-destaque-{!! $identificador !!}")[0].scrollWidth - $(
                    "#slide-lotes-destaque-{!! $identificador !!}")[0].clientWidth;
            ...
            $("#slide-lotes-destaque-{!! $identificador !!}-left").click(function() {
                $("#slide-lotes-destaque-{!! $identificador !!}").animate({
                    scrollLeft: $("#slide-lotes-destaque-{!! $identificador !!}").scrollLeft() -
                        item_width
                }, function() {
                    updateButtons();
                });
