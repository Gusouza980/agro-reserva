                    ...
                    <table class="" style="width: 700px; margin: 0 auto; margin-top: 30px; font-size: 9px; border-collapse: collapse;">
                        <thead style="background-color: black; color: white; padding: 3px 0px;">
                            <tr>
                                ...
                                <td class="td-border" colspan="3" style="text-align: center; font-weight: bold; padding: 3px 0px;">{{$produto->produtable->nome}}</td>
                            </tr>
                        </thead>
                        <tbody>
                            <tr style="text-align:center;">
                                <td class="td-border" style="padding: 4px 0px;">{{str_pad($produto->produtable->numero, 3, "0", STR_PAD_LEFT)}}{{$produto->produtable->letra}}</td>
                                <td class="td-border" style="padding: 4px 0px;" colspan="2">RGD: {{$produto->produtable->registro}} - GPTA: {{$produto->produtable->gpta}} - NASC.: @if($produto->produtable->nascimento) {{date("d/m/Y", strtotime($produto->produtable->nascimento))}} @endif - CCG: {{$produto->produtable->ccg}} - FÊMEA</td>
                                <td class="td-border" style="padding: 4px 0px; width: 170px;">@if($produto->produtable->parto) Último parto em {{date("d/m/Y", strtotime($produto->produtable->parto))}} @endif</td>
                            </tr>
                            <tr style="text-align:center;">
                                ...
                                <td class="td-border" style="padding: 4px 0px; width: 170px;">
                                    <table style="width: 100%;">
                                        <tbody style="text-align: center;">
                                            <tr>
                                                <td>Valor Total:</td>
                                                <td><b>R${{number_format($produto->produtable->preco + ($venda->comissao / $venda->carrinho->produtos->count()) - ($venda->desconto / $venda->carrinho->produtos->count()), 2, ",", ".")}}</b></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </td>
                                    ...
                                    <table style="width: 100%;">
                                        ...
                                        <tbody style="text-align: center;">
                                            ...
                                            @for($i = 1; $i <= $metade_parcelas; $i++)
                                                @php
                                                    $parcela = $venda->getRelationValue("parcelas")->where("numero", $i)->first();
                                                @endphp
                                                <tr>
                                                    <td>{{$i}}</td>
