...
class InfosLotesDestaque extends Component
{
    ...
    public function mount(){
        ...
        // if(!count($this->lotes)){
            $this->lotes = Lote::with("fazenda:id,nome_fazenda,logo,logo_evento,slug")->with("raca:id,nome")->with("produto")->with("reserva:id,desconto,max_parcelas")->select("id", "reserva_id", "fazenda_id", "nome", "numero", "registro", "nascimento", "raca_id", "sexo", "video", "beta_caseina", "liberar_compra")->whereHas("reserva", function($q) {
                $q->where([["encerrada", false], ['aberto', true]]);
