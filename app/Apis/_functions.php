<?php
	...
	if(!session()->get("userid")){
		session()->put(['lojaid' => $shopcode]);
		session()->put(['userid' => 0]);
		session()->put(['username' => '']);
		session()->put(['useruuid' => getUUID()]);
		// $_SESSION["lojaid"] = $shopcode;
		// $_SESSION["userid"] = 0;
		// $_SESSION["username"] = '';
		// $_SESSION["useruuid"] = getUUID();
		setcookie("loja-id", session()->get('lojaid'), (time() + (30 * 86400)), "/");
		setcookie("user-id", session()->get('userid'), (time() + (30 * 86400)), "/");
		setcookie("user-name", session()->get('username'), (time() + (30 * 86400)), "/");
		setcookie("user-uuid", session()->get('useruuid'), (time() + (30 * 86400)), "/");
	...
	// if(isset($_COOKIE['user-id']) && ($_COOKIE['user-id'] == 0)) {
